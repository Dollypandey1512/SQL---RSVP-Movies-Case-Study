...
-- Type your code below:
   ...
   SELECT COUNT(*) AS Total_row_of_genre FROM genre;
   SELECT COUNT(*) AS Total_row_of_movie FROM movie;
   SELECT COUNT(*) AS Total_row_of_names FROM names;
   SELECT COUNT(*) AS Total_row_of_ratings FROM ratings;
   SELECT COUNT(*) AS Total_row_of_role_mapping FROM role_mapping;
...
-- Type your code below:
   SELECT count(*) from movie where title is NULL or title='';
   SELECT count(*) from movie where id is NULL or id='';
   SELECT count(*) from movie where year is NULL or year='';
   SELECT count(*) from movie where date_published is NULL;
   SELECT count(*) from movie where duration is NULL;
   SELECT count(*) from movie where country is NULL or title='';
   SELECT count(*) FROM movie WHERE worlwide_gross_income IS NULL OR worlwide_gross_income='';
...
-- output for the first part and second part of this question respectively.
...
SELECT title,
		avg_rating,
        RANK() OVER(window_rating) as movie_rank 
FROM ratings as r
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
SELECT production_company, no_of_movies, 
       DENSE_RANK() OVER (ORDER BY no_of_movies DESC) AS prod_company_rank
FROM (
    SELECT m.production_company, COUNT(m.id) AS no_of_movies
    FROM movie m
...
+---------------+-------------------+ */
-- Type your code below:
SELECT g.genre, COUNT(m.id) AS count_movies 
FROM movie AS m 
INNER JOIN genre AS g ON m.id = g.movie_id 
INNER JOIN ratings AS r ON m.id = r.movie_id WHERE m.year = 2017 AND MONTH(m.date_published) = 3 AND m.country LIKE'USA' AND r.total_votes>1000 
GROUP BY g.genre 
...
ON m.id= r.movie_id
WHERE (date_published between '2018-04-01' and '2019-04-01') and median_rating=8  -- filtering for movies released between 1  
...
-- Type your code below:
SELECT sum(total_votes) as counting_votes     -- count of German movies
FROM movie as m 
INNER JOIN ratings as r                   -- joining movies and ratings tables
ON m.id= r.movie_id
WHERE country regexp 'Germany'; 

SELECT sum(total_votes) as counting_votes     -- count of Italian movies
FROM movie as m 
INNER JOIN ratings as r                   -- joining movies and ratings tables
ON m.id= r.movie_id
WHERE country regexp 'Italy';

-- AS we see answer is absolutely YES in this case.
...
SELECT director_name, COUNT(DISTINCT movie_id) AS movie_count
FROM (
    ...
    WHERE r.avg_rating > 8 
      AND g.genre IN ('Drama', 'Comedy', 'Thriller')
) AS directors_filtered
...
FROM   thriller_movies order by avg_rating desc ; 
